...
exports.runQuery = function (query, list) {
    var result = list;
    for (var i of query) {
        result = result.filter(function (j) {
            let testPassed = false;
            let queryVal = i.value,
                testTarget = j[i.key];
            if (testTarget === null || typeof testTarget === "undefined") {
                ...
                if (!i.caseSensitive) {
                    ...
                    testTarget = testTarget.constructor.name === "Array" ? testTarget.map((i) => {
                        return i.toLowerCase();
                    }) : (testTarget.toLowerCase ? testTarget.toLowerCase() : testTarget);
                }
                if (i.type == 'contains') {
                    ...
                    testPassed = (testTarget.constructor.name === "Array" && testTarget.some((k) => {
                        return queryVal.test(k);
                    })) || queryVal.test(testTarget);
                } else {
